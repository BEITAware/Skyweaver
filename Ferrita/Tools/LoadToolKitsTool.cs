...
{
    public sealed class LoadToolKitsTool : IFerritaTool, IFerritaToolInvocationPresentationProvider, IFerritaToolPromptDescriptionProvider
    {
        ...
        public LoadToolKitsTool()
            : this(new FerritaToolKitService(), new FerritaPromptToolCatalogService(), new FerritaToolManager())
        {
        }
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            var callableTools = context.CurrentAgent == null
                ? Array.Empty<FerritaPromptToolDefinition>()
        ...
        private static string BuildResultContent(
            IReadOnlyList<FerritaToolKitDefinition> loadedToolKits,
            IReadOnlyList<FerritaPromptToolDefinition> callableTools,
            ...
            IReadOnlyList<string> ambiguousToolKitNames)
        {
            ...
            if (loadedToolKits.Count > 0)
            {
                ...
                if (callableTools.Count > 0)
                {
                    ...
                    FerritaToolPromptSupport.AppendToolListing(builder, callableTools);
                }
                else
                {
        ...
        private static string GetToolKitDisplayName(FerritaToolKitDefinition toolKit)
        {
