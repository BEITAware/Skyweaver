            ...
            var thread = new Thread(() =>
            {
                try
                {
                    ...
                    if (ClipboardAccessService.TryGetImage(out BitmapSource? image, out string? errorMessage) && image != null)
                    {
                        ...
                        var xmlReturn = $"<FerritaPreservedContent><Image Path=\"{System.Security.SecurityElement.Escape(targetFilePath)}\" /></FerritaPreservedContent>\nClipboard image captured successfully.";
                        tcs.TrySetResult(FerritaToolResult.Success(xmlReturn));
                    }
                    else
                    {
                        tcs.TrySetResult(FerritaToolResult.Failure(errorMessage ?? "The clipboard does not contain a valid image."));
                    }
                }
                ...
                catch (Exception ex)
                {
                    ...
                    tcs.TrySetResult(FerritaToolResult.Failure($"Failed to read clipboard image: {ex.Message}"));
                }
            });
