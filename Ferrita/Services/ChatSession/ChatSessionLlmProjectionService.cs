...
namespace Ferrita.Services.ChatSession
{
    ...
    public sealed class ChatSessionLlmProjectionService
    {
        ...
        private static string BuildTextContent(ChatSessionTranscriptEntry entry, ChatSessionTranscriptBlock block)
        {
            ...
            {
                return FerritaPreservedTextContentXml.TryParse(content, out var textContent)
                    ? textContent.Text
        ...
        {
            return content.StartsWith("<FerritaPreservedContent", StringComparison.OrdinalIgnoreCase)
                ? content
                : $"<FerritaPreservedContent>{content}</FerritaPreservedContent>";
        }
