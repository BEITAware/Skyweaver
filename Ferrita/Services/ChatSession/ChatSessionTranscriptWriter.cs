...
namespace Ferrita.Services.ChatSession
{
    ...
    public sealed class ChatSessionTranscriptWriter
    {
        ...
        private static FerritaToolResultPresentationHints ResolveToolOutputPresentationHints(
            IReadOnlyList<FerritaToolReturnPayload> toolReturns)
        {
            return toolReturns
                ...
                ?? FerritaToolResultPresentationHints.None;
        }
            ...
            FerritaToolResultPresentationHints presentationHints)
        {
            ...
            {
                block.Metadata[FerritaToolResultPresentationMetadataKeys.PresentationKind] =
        ...
        {
            SetMetadata(block.Metadata, FerritaToolProgressMetadataKeys.Phase, progress.Phase);
            SetMetadata(block.Metadata, FerritaToolProgressMetadataKeys.StatusText, progress.StatusText);
            SetMetadata(
                block.Metadata,
                FerritaToolProgressMetadataKeys.CompletedItems,
                progress.CompletedItems?.ToString(CultureInfo.InvariantCulture));
            SetMetadata(
                block.Metadata,
                FerritaToolProgressMetadataKeys.TotalItems,
                progress.TotalItems?.ToString(CultureInfo.InvariantCulture));
            SetMetadata(
                block.Metadata,
                FerritaToolProgressMetadataKeys.ProgressFraction,
                progress.ProgressFraction?.ToString("R", CultureInfo.InvariantCulture));
            SetMetadata(
                block.Metadata,
                FerritaToolProgressMetadataKeys.IsCompleted,
                progress.IsCompleted ? "true" : "false");
            SetMetadata(
                block.Metadata,
                FerritaToolProgressMetadataKeys.ActiveItems,
