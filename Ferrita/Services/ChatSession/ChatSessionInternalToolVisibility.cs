...
namespace Ferrita.Services.ChatSession
{
    public static class ChatSessionInternalToolVisibility
    {
        ...
        {
            return xml.IndexOf($"{attributeName}=\"{FerritaBuiltInToolNames.Passdown}\"", StringComparison.OrdinalIgnoreCase) >= 0 ||
                   xml.IndexOf($"{attributeName}='{FerritaBuiltInToolNames.Passdown}'", StringComparison.OrdinalIgnoreCase) >= 0 ||
                   xml.IndexOf($"{attributeName}=\"{FerritaBuiltInToolNames.WaitForAsyncTools}\"", StringComparison.OrdinalIgnoreCase) >= 0 ||
                   xml.IndexOf($"{attributeName}='{FerritaBuiltInToolNames.WaitForAsyncTools}'", StringComparison.OrdinalIgnoreCase) >= 0 ||
                   xml.IndexOf($"{attributeName}=\"{FerritaBuiltInToolNames.GetAsyncToolProgress}\"", StringComparison.OrdinalIgnoreCase) >= 0 ||
                   xml.IndexOf($"{attributeName}='{FerritaBuiltInToolNames.GetAsyncToolProgress}'", StringComparison.OrdinalIgnoreCase) >= 0;
        }
    }
}
