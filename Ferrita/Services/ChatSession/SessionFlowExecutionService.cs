...
namespace Ferrita.Services.ChatSession
{
    public sealed class SessionFlowExecutionService
    {
        ...
        private sealed class NodeExecutionOutcome
        {
            ...
            public string? CustomCompletedMessage { get; init; }
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            var pendingContextInjections = new List<LanguageModelChatMessage>();
            ...
            while (readyQueue.Count > 0)
            {
                ...
                await PublishAsync(
                    ...
                    CreateRuntimeEvent(
                        ...
                        compiledNode,
                        message: outcome.IsSkipped ? "节点未被激活，已跳过。" : (outcome.CustomCompletedMessage ?? "节点执行完成。"),
        ...
        private async Task<NodeExecutionOutcome> ExecuteNodeAsync(
            ...
            List<LanguageModelChatMessage> pendingContextInjections,
            ...
            CancellationToken cancellationToken)
        {
            ...
            return node.Kind switch
            {
                ...
                SessionFlowNodeKind.Agent => await ExecuteAgentNodeAsync(
                    ...
                    onEventAsync,
                    cancellationToken).ConfigureAwait(false),
                SessionFlowNodeKind.ContextInjection => ExecuteContextInjectionNode(
                    compiledNode,
                    deliveredBindings,
                    pendingContextInjections),
            ...
            CancellationToken cancellationToken)
        {
            ...
            if (pendingContextInjections.Count > 0)
            {
                foreach (var injection in pendingContextInjections)
                {
                    agentHistory.Add(injection.Clone());
                }
                pendingContextInjections.Clear();
            }
        ...
        private static NodeExecutionOutcome ExecuteContextInjectionNode(
            SessionFlowCompiledNode compiledNode,
            IReadOnlyList<DeliveredInputBinding> deliveredBindings,
            List<LanguageModelChatMessage> pendingContextInjections)
        {
            var node = compiledNode.Node;
            var content = node.ContextInjectionContent;
            if (!string.IsNullOrWhiteSpace(content))
            {
                pendingContextInjections.Add(new LanguageModelChatMessage(
                    node.ContextInjectionRole,
                    content));
            }
            ...
            if (!string.IsNullOrWhiteSpace(content))
            {
                customMessage = $"已注入 [{node.ContextInjectionRole}] 上下文内容：{content}";
            }

            return new NodeExecutionOutcome
            {
                ExplicitOutputPayloads = explicitOutputs,
                CustomCompletedMessage = customMessage
            };
        }
        ...
        private static FerritaToolContext BuildToolContext(SessionFlowExecutionRequest request)
        {
        ...
        private static ChatSessionRuntimeEvent CreateRuntimeEvent(
            ...
            FerritaToolInvocation? toolInvocation = null,
            FerritaStreamingToolCallSnapshot? toolCallSnapshot = null,
            string? toolXml = null,
            string? toolOutputXml = null,
            IReadOnlyList<FerritaToolReturnPayload>? toolReturns = null,
            FerritaToolProgressUpdate? toolProgress = null,
            ...
            bool isPayloadFromPassdown = false)
        {
            return new ChatSessionRuntimeEvent
            {
                ...
                ToolReturns = toolReturns ?? Array.Empty<FerritaToolReturnPayload>(),
