...
{
    public sealed class FerritaToolInvocationStreamingParser
    {
        private static readonly Regex s_attributePattern = new(
            @"(?<name>[A-Za-z_][A-Za-z0-9_.:-]*)\s*=\s*(?:""(?<double>[^""]*)""|'(?<single>[^']*)')",
            RegexOptions.Compiled | RegexOptions.IgnoreCase);

        private readonly IReadOnlyDictionary<string, FerritaToolDefinition> _toolDefinitions;

        public FerritaToolInvocationStreamingParser(IEnumerable<FerritaToolDefinition> toolDefinitions)
        {
            _toolDefinitions = (toolDefinitions ?? Array.Empty<FerritaToolDefinition>())
                .Where(definition => definition != null)
                .GroupBy(definition => definition.Name, StringComparer.OrdinalIgnoreCase)
                .ToDictionary(group => group.Key, group => group.Last(), StringComparer.OrdinalIgnoreCase);
        }

        public IReadOnlyList<FerritaStreamingToolCallSnapshot> Parse(
            string? rawContent,
            ...
            int initialToolCallIndex = 0)
        {
            ...
            {
                return Array.Empty<FerritaStreamingToolCallSnapshot>();
            }

            var snapshots = new List<FerritaStreamingToolCallSnapshot>();
            var searchIndex = 0;
            ...
            while (searchIndex < rawContent.Length)
            {
                ...
                if (!string.IsNullOrWhiteSpace(normalizedToolName))
                {
                    ...
                    IReadOnlyList<FerritaStreamingToolParameterSnapshot> parameters;
                    int toolEndIndex;
                    bool isInvocationClosed;

                    if (!hasCompleteOpenTag)
                    {
                        parameters = Array.Empty<FerritaStreamingToolParameterSnapshot>();
                    ...
                    else
                    {
                        parameters = ParseParameterizedToolBody(
                            rawContent,
                            toolDefinition?.Parameters ?? Array.Empty<FerritaToolParameterDefinition>(),
                            toolOpenTagEndIndex,
                            toolElementName,
                            out toolEndIndex,
                    ...
                    snapshots.Add(new FerritaStreamingToolCallSnapshot
                    {
        ...
        private static IReadOnlyList<FerritaStreamingToolParameterSnapshot> ParseParameterizedToolBody(
            string rawContent,
            IReadOnlyList<FerritaToolParameterDefinition> parameterDefinitions,
            int bodyStartIndex,
        ...
        private static IReadOnlyList<FerritaStreamingToolParameterSnapshot> BuildParameterSnapshots(
            IEnumerable<ParameterBuilder> parameters)
        {
            return parameters.Select(parameter => new FerritaStreamingToolParameterSnapshot
                {
                    Name = parameter.Name,
