...
{
    public enum FerritaLineDiffEntryKind
    {
        Anchor = 0,
        Added = 1,
        Removed = 2,
    ...
    public sealed class FerritaLineDiffEntry
    {
        public FerritaLineDiffEntry(
            string lineNumberText,
            string marker,
            string text,
            FerritaLineDiffEntryKind kind)
        {
        ...
        public FerritaLineDiffEntryKind Kind { get; }

        public string KindName => Kind.ToString();

        public bool IsSeparator => Kind == FerritaLineDiffEntryKind.Separator;
    }

    public static class FerritaLineDiffPresentation
    {
        ...
        public static bool TryParseToolOutputXml(
            ...
            out IReadOnlyList<FerritaLineDiffEntry> entries)
        {
        ...
        public static bool TryParseContent(
            ...
            out IReadOnlyList<FerritaLineDiffEntry> entries)
        {
            entries = Array.Empty<FerritaLineDiffEntry>();
            if (string.IsNullOrWhiteSpace(content))
            {
                return false;
            }

            var parsedEntries = new List<FerritaLineDiffEntry>();
            var lines = EnumerateRawLines(content);
            foreach (var rawLine in lines)
            {
                if (rawLine.Length == 0)
                {
                    parsedEntries.Add(new FerritaLineDiffEntry(string.Empty, string.Empty, string.Empty, FerritaLineDiffEntryKind.Separator));
                    continue;
                }
                ...
                parsedEntries.Add(new FerritaLineDiffEntry(
            ...
            entries = parsedEntries.Count == 0
                ? Array.Empty<FerritaLineDiffEntry>()
        ...
        private static FerritaLineDiffEntryKind ParseKind(string marker)
        {
            return marker switch
            {
                "+" => FerritaLineDiffEntryKind.Added,
                "-" => FerritaLineDiffEntryKind.Removed,
                _ => FerritaLineDiffEntryKind.Anchor
            };
        }
