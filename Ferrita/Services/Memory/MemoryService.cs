...
namespace Ferrita.Services.Memory
{
    public sealed class MemoryService
    {
        private const int RetrievalCandidateCount = 120;
        private const string MemoryCollectionName = "Ferrita.Memory.Blocks";
        private const string VectorsDatabaseName = "Vectors";
        private const string PreservedContentStartToken = "<FerritaPreservedContent";
        private const string PreservedContentEndToken = "</FerritaPreservedContent>";
        private const string EscapedPreservedContentStartToken = "&lt;FerritaPreservedContent";
        private const string EscapedPreservedContentEndToken = "&lt;/FerritaPreservedContent&gt;";
        ...
        {
            content = default;
            if (FerritaPreservedTextContentXml.TryParse(fragment, out var textContent))
            {
                ...
                content = new PreservedMemoryContent(
                    ...
                    FerritaPreservedTextContentXml.Build(
                        text,
                        textContent.Name,
                        textContent.Path,
            ...
            try
            {
                ...
                if (!string.Equals(root.Name.LocalName, "FerritaPreservedContent", StringComparison.OrdinalIgnoreCase))
                {
                    return false;
                }
        ...
        private static string ResolveTextForEmbedding(string? content)
        {
            ...
            return FerritaPreservedTextContentXml.TryParse(normalizedContent, out var preservedText)
            ...
            string? mediaType)
        {
            ...
            return new XElement("FerritaPreservedContent", element).ToString(SaveOptions.DisableFormatting);
        }
            ...
            IReadOnlyList<RetrievalResult> results)
        {
            ...
            var root = new XElement(
                "FerritaPreservedContent",
                new XComment("Relevant memory from previous conversations between you and the user. This preserved content block is injected by Ferrita and is only visible to the LLM. The user must never see this block directly."),
                memoryElement);
        ...
        private sealed class MemoryLayout
        {
            ...
            public static MemoryLayout CreateDefault()
            {
                return new MemoryLayout(Path.Combine(
                    FerritaDirectoryRuntime.Instance.AerialCityDirectoryPath,
                    RootFolderName));
            }
        }
