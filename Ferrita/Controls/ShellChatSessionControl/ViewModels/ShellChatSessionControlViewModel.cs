...
using Ferrita.Controls.ChatSessionControl.Services;
...
namespace Ferrita.Controls.ShellChatSessionControl.ViewModels
{
    public sealed class ShellChatSessionControlViewModel : ObservableObject
    {
        private static readonly TimeSpan StreamingProjectionRefreshInterval = TimeSpan.FromMilliseconds(32);
        private const string ShellIdleAvatarPath = "pack://application:,,,/Resources/FerritaAppLogo.png";
        ...
        private async Task SendMessageAsync()
        {
            ...
            PromptHistoryService.Instance.AddHistoryEntry(trimmedText, "Shell");
        ...
        private static string BuildShellContextText(ShellChatStartupContext context)
        {
            ...
            var snapshotElement = new XElement(
                ...
                new XElement(
                    "Instruction",
                    "The user opened Ferrita from Windows Explorer. Treat this as one-turn shell context for the current user request only. Items marked TransferMode=\"PathOnly\" are path references, not text or multimodal content; do not read, inline, or execute them unless the user explicitly asks and an appropriate confirmed tool is used."),
                contextElement,
