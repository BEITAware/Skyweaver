...
namespace Skyweaver.Tools
{
        ...
        ISkyweaverToolPromptDescriptionProvider
    {
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            {
                return SkyweaverToolResult.Failure("Urls 参数不能为空。");
            }

            var urls = ParseUrls(urlsText);
            if (urls.Count == 0)
            {
                return SkyweaverToolResult.Failure("无法从 Urls 参数中提取出有效的网页 URL。");
            }
            ...
            try
            {
                ...
                return successfullyBrowsed > 0
                    ...
                    : SkyweaverToolResult.Failure("所有指定的网页均未能访问成功。\n\n" + content, data);
            }
            catch (Exception ex)
            {
                return SkyweaverToolResult.Failure($"网页浏览执行时遇到异常: {ex.Message}");
            }
        }
