...
using Skyweaver.Models.Search;
using Skyweaver.Services.Search;
...
namespace Skyweaver.Tools
{
        ...
        ISkyweaverToolPromptDescriptionProvider
    {
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            {
                return SkyweaverToolResult.Failure("Queries 参数不能为空。");
            }

            var queries = ParseQueries(queriesText);
            if (queries.Count == 0)
            {
                return SkyweaverToolResult.Failure("无法从 Queries 参数中提取出有效的搜索查询词。");
            }
            ...
            try
            {
                ...
                return SkyweaverToolResult.Success(content, data);
            }
            catch (Exception ex)
            {
                return SkyweaverToolResult.Failure($"网页搜索失败: {ex.Message}");
            }
        }
