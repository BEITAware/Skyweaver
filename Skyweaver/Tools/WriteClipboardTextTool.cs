        ...
        ISkyweaverToolPromptDescriptionProvider
    {
            ...
            CancellationToken cancellationToken = default)
        {
            try
            {
                ...
                if (ClipboardAccessService.TrySetText(text, out string? errorMessage))
                {
                    return Task.FromResult(SkyweaverToolResult.Success($"Successfully wrote {text.Length} characters to the clipboard."));
                }
                else
                {
                    return Task.FromResult(SkyweaverToolResult.Failure(errorMessage ?? "Failed to write to clipboard."));
                }
            }
            ...
            catch (Exception ex)
            {
                ...
                return Task.FromResult(SkyweaverToolResult.Failure($"Failed to write to clipboard: {ex.Message}"));
            }
        }
    }
}
