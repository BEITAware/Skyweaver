...
namespace Skyweaver.ViewModels
{
    ...
    /// </summary>
    public class SkyweaverGroundingEngineViewModel : ObservableObject
    {
        ...
        public SkyweaverGroundingEngineViewModel()
        {
            // 加载初始模拟数据以供界面预览
            LoadDefaultData();
            LoadPendingData();
        ...
        private void LoadPendingData()
        {
            ...
            PendingGroundings.Add(new PendingGroundingItem
            {
                ...
                StatusText = "正在匹配本地知识库 (s:\\ProjectSkyweaver\\docs)...",
                Progress = 0.80,
                EtaText = "剩余约 1.2s"
            });
        ...
        private void LoadDefaultData()
        {
            ...
            {
                ChatMessage = "Skyweaver 是一款专门为分布式协作 and 智能辅助设计的现代开发平台，支持实时事实比对引擎。",
                FactMessage = "证实：Skyweaver 产品规格书（Sec 1.2）中指明其核心子系统为分布式语义比对器与实时辅助窗口。",
