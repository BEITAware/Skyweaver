...
{
    public sealed class SkyweaverToolArguments
    {
        private readonly Dictionary<string, SkyweaverToolArgumentValue> _values;

        private SkyweaverToolArguments(
            IEnumerable<SkyweaverToolArgumentValue> values,
            IReadOnlyDictionary<string, string?> rawArguments)
        {
            _values = new Dictionary<string, SkyweaverToolArgumentValue>(StringComparer.OrdinalIgnoreCase);
        ...
        public IReadOnlyCollection<SkyweaverToolArgumentValue> Values => _values.Values;

        internal static SkyweaverToolArguments Bind(
            IReadOnlyList<SkyweaverToolParameterDefinition> definitions,
            IReadOnlyDictionary<string, string?>? rawArguments)
        {
            ...
            var values = new List<SkyweaverToolArgumentValue>(definitions.Count);
            foreach (var definition in definitions)
            {
                ...
                if (string.IsNullOrWhiteSpace(candidate))
                {
                    ...
                    values.Add(new SkyweaverToolArgumentValue(definition, rawValue, null));
                    continue;
                }

                values.Add(new SkyweaverToolArgumentValue(
                    definition,
                    rawValue,
                    ConvertValue(definition, candidate)));
            }

            return new SkyweaverToolArguments(values, normalizedRawArguments);
        }
        ...
        public SkyweaverToolArgumentValue? GetValue(string name)
        {
        ...
        private static object ConvertValue(SkyweaverToolParameterDefinition definition, string rawValue)
        {
            try
            {
                ...
                {
                    SkyweaverToolParameterType.Boolean => (object)ParseBoolean(rawValue),
                    SkyweaverToolParameterType.Integer => ParseInteger(rawValue),
                    SkyweaverToolParameterType.Number => ParseNumber(rawValue),
                    SkyweaverToolParameterType.Json => ParseJson(rawValue),
                    _ => rawValue
                };
            }
    ...
    public sealed class SkyweaverToolArgumentValue
    {
        internal SkyweaverToolArgumentValue(
            SkyweaverToolParameterDefinition definition,
            string? rawValue,
            object? value)
        {
        ...
        public SkyweaverToolParameterDefinition Definition { get; }
