...
{
    public sealed class ChatRibbonBackgroundEffect : ShaderEffect
    {
...
float3 shiftHueAndLightness(float3 color)
{
    float3 hsl = rgbToHsl(saturate(color));
    return hslToRgb(hsl);
}
...
float4 main(float2 uv : TEXCOORD) : COLOR
{
    ...
    float3 baseColor = lerp(float3(0.04, 0.08, 0.24), float3(0.18, 0.48, 0.77), gradientMix);
    baseColor *= 0.62 + vignette * 0.34;
    baseColor += float3(0.06, 0.08, 0.19) * topGlow;
    baseColor += float3(0.06, 0.1, 0.22) * rightGlow * 0.45;

    float haze = (noise(p * 2.15 + float2(time * 0.04, -time * 0.03)) - 0.5) * 0.06;
    baseColor += haze;

    float baseLuma = dot(baseColor, float3(0.2126, 0.7152, 0.0722));
    baseColor = lerp(float3(baseLuma, baseLuma, baseLuma), baseColor, 0.95);

    float2 diag = float2(p.x * 0.84 - p.y * 0.58, p.x * 0.58 + p.y * 0.84);
    float2 diag2 = float2(p.x * 0.93 + p.y * 0.36, -p.x * 0.36 + p.y * 0.93);
    ...
    float3 ribbonColor = float3(0.42, 0.68, 0.94) * (r1 * 0.32 + r2 * 0.28);
    ribbonColor += float3(0.64, 0.76, 0.96) * r3 * 0.24;
    ribbonColor += float3(0.32, 0.58, 0.92) * r4 * 0.14;
    ribbonColor += float3(0.2, 0.42, 0.70) * (sweepA * 0.14 + sweepB * 0.1);
    ribbonColor += float3(0.88, 0.92, 0.99) * glint * 0.08;

    float3 meshColor = float3(0.38, 0.62, 0.96) * dots * 0.2;

    float3 finalColor = baseColor + ribbonColor + meshColor;
    finalColor = finalColor / (1.0 + finalColor * 0.14);
    float finalLuma = dot(finalColor, float3(0.2126, 0.7152, 0.0722));
    finalColor = lerp(float3(finalLuma, finalLuma, finalLuma), finalColor, 0.95);
    finalColor *= 0.84;
    finalColor = shiftHueAndLightness(finalColor);
    ...
    return float4(finalColor * max(source.a, 0.0001), source.a);
        ...
        public static readonly DependencyProperty InputProperty = RegisterPixelShaderSamplerProperty(
            nameof(Input),
            typeof(ChatRibbonBackgroundEffect),
            0);
        ...
        public ChatRibbonBackgroundEffect()
        {
