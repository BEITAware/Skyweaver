    ...
    public partial class PathSelectionPage : UserControl
    {
        ...
        private void InitializeDefaultPaths()
        {
            try
            {
                ...
                InstallPath = Path.Combine(programFiles, "Skyweaver");
                InstallPathTextBox.Text = InstallPath;

                // 默认工作文件夹路径
                string documentsPath = Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments);
                WorkFolderPath = Path.Combine(documentsPath, "Skyweaver");
                WorkFolderTextBox.Text = WorkFolderPath;

                // 默认脚本文件夹路径
                ScriptsFolderPath = Path.Combine(documentsPath, "Skyweaver", "Scripts");
        ...
        private void BrowseInstallPathButton_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                if (dialog.ShowDialog() == System.Windows.Forms.DialogResult.OK)
                {
                    ...
                    // 如果选择的是根目录，自动添加 \Skyweaver\
                    if (IsRootDirectory(selectedPath))
                    {
                        selectedPath = Path.Combine(selectedPath, "Skyweaver");
                    }
