        ...
        private async Task RegisterStartMenuShortcut()
        {
            await Task.Run(() =>
            {
                try
                {
                    ...
                    string skyweaverFolderPath = Path.Combine(programsPath, "Skyweaver");

                    // 创建Skyweaver文件夹
                    Directory.CreateDirectory(skyweaverFolderPath);
                    LogMessage("已创建开始菜单文件夹");

                    // 创建快捷方式
                    string shortcutPath = Path.Combine(skyweaverFolderPath, "Skyweaver.lnk");
                    string targetPath = Path.Combine(PathSelectionPage.InstallPath, "Skyweaver.exe");

                    CreateShortcut(shortcutPath, targetPath, PathSelectionPage.InstallPath, "Skyweaver - 专业图像处理和色彩管理工具");
                    LogMessage("已创建开始菜单快捷方式");

                    // 创建卸载快捷方式
                    string uninstallShortcutPath = Path.Combine(skyweaverFolderPath, "卸载 Skyweaver.lnk");
                    ...
                    {
                        CreateShortcut(uninstallShortcutPath, uninstallTargetPath, PathSelectionPage.InstallPath, "卸载 Skyweaver");
